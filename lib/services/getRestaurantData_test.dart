...
class NearbyRestaurantData {
  ...
  Future<String> getPhotoUrl(dynamic place,
      {int maxWidth = 400, int maxHeight = 400}) async {
    if (place['photos'] != null && place['photos'].isNotEmpty) {
      String photoName = place['photos'][0]['name'] as String;
      if (photoName.isNotEmpty) {
        var url = Uri.parse(
            'https://places.googleapis.com/v1/$photoName/media?maxHeightPx=$maxHeight&maxWidthPx=$maxWidth&key=$googApikey');
  ...
  Future<List<Map<String, dynamic>>> fetchData({
    ...
    try {
      print('Fetching nearby restaurant data with radius: $radius & restaurantType: $restaurantType');
      http.StreamedResponse response = await request.send();
