...
class NearbyRestaurantData {
  ...
  Future<List<Map<String, dynamic>>> fetchData(BuildContext context) async {
    ...
    // 获取最新的筛选条件
    double radius = filterProvider.apiRadius ?? 3000;
        ...
        return (restaurantData['places'] as List<dynamic>)
          ...
          String getPhotoUrl(dynamic place, {int maxWidth = 400, int maxHeight = 400}) {
            if (place['photos'].isNotEmpty) {
              print('photo name is not empty.');
              String photoName = place['photos'][0]['name'];
              
              return 'https://places.googleapis.com/v1/$photoName/media?key=$googApikey&maxHeightPx=$maxHeight';
